...
select public.finish_openai_model_sync_run(:'sync_run_id'::uuid, 'complete', 3, 1, 1, 1);
...
select public.finish_openai_model_sync_run(:'failed_run_id'::uuid, 'failed', 1, 0, 0, 0, 'catalog_reconciliation_failed', 'secret Authorization header must not be retained');
