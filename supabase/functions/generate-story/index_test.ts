...
Deno.test("container hardCap constrains maxCompletionTokens", async () => {
  const { store: creditStore } = makeMockCreditStore(makeEntitlement());
  const { store: rateLimitStore } = makeMockRateLimitStore({ allowed: true });
  const { store: generationModelStore } = makeMockGenerationModelStore();
  const { store: persistenceStore } = makeMockPersistenceStore();
  ...
  const provider: LLMProvider = {
    complete(_msgs: unknown[], maxTokens: number) {
  ...
  // Beat container has hardCap 350. The server should clamp maxTokens
  // to at most 350 regardless of the client's requested outputBudget.
  await handler(makeAuthRequest(makeBaseRequest({
    container: "beat",
    outputBudget: 5000, // client requests 5000, but container hardCap is 350
  })), {
    provider,
    creditStore,
    rateLimitStore,
  ...
  assertExists(capturedMaxTokens);
  if (capturedMaxTokens !== null) {
    assertEquals(capturedMaxTokens <= 350, true);
  }
});
