...
export async function processSectionMemory(
  ...
  // Step 3: extract the scene memory via LLM unless the caller is the
  // outline Accept All indexer. Suggestions are not prose scenes and should
  // not trigger a full scene-memory extraction (or its billable LLM call).
  let sceneMemory: SceneMemory;
  if (options.extractMemory === false) {
    sceneMemory = {
      extracted_summary: body.summary ?? body.raw_text ?? "",
      character_deltas: [],
      plot_thread_deltas: [],
      continuity_facts: [],
      open_loops: [],
    ...
    if (!sceneMemory.extracted_summary) {
      throw new SectionEmbeddingError(
        "invalid_request",
        "Cannot index a section without summary text",
      );
    }
    console.log(
      `[embed-section] memory extraction skipped for outline index section=${body.outline_section_id}`,
    );
  } else {
...
${body.raw_text}`
      ...
      try {
        ...
        console.error(
          `[embed-section] llm_prompts insert failed: ${(logErr as Error).message}`,
        );
      }
      ...
      try {
        ...
        console.error(
          `[embed-section] LLM extraction returned invalid JSON: ${String(parseErr)} raw=${raw.slice(0, 300)}`,
        );
        throw new SectionEmbeddingError(
          "provider_error",
  ...
  console.log(
    `[embed-section] memory ready summary_len=${sceneMemory.extracted_summary.length} layers=${sceneMemory.character_deltas.length || sceneMemory.plot_thread_deltas.length || sceneMemory.continuity_facts.length || sceneMemory.open_loops.length || Object.keys(sceneMemory.scene_ending_state).length ? 6 : 0}`,
  );
