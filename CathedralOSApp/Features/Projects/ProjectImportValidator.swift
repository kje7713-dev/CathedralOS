...
struct ImportValidationErrors: Error {
...
enum ProjectImportValidator {
    ...
    static func validate(jsonString: String) -> Result<(ProjectImportExportPayload, [ImportValidationIssue]), ImportValidationErrors> {
        guard let data = jsonString.data(using: .utf8) else {
            return .failure(ImportValidationErrors(issues: [ImportValidationIssue(severity: .error, message: "Invalid JSON: Could not encode string as UTF-8.")]))
        }
        ...
        do {
            ...
            return .failure(ImportValidationErrors(issues: [ImportValidationIssue(severity: .error, message: "Invalid JSON: \(error.localizedDescription)")]))
        }

        guard payload.schema == "cathedralos.project_schema" else {
            return .failure(ImportValidationErrors(issues: [ImportValidationIssue(
                severity: .error,
        ...
        if !errors.isEmpty {
            return .failure(ImportValidationErrors(issues: errors))
        }
